...
from toolbox.models.MGSGNet.segformer.mix_transformer import mit_b4
...
class SGM_t(nn.Module):
    def __init__(self, channel,inchannels = [512, 320, 128, 64]):
        super(SGM_t, self).__init__()
        self.guideLayers=4
        self.reduceDim=48
        self.reduceBot = nn.Conv2d(channel, self.reduceDim, kernel_size=1, bias=False)
        ...
        self.HR = nn.ModuleList([nn.Conv2d(channel, 48, kernel_size=1, bias=False)
                                       for i in range(self.guideLayers)])
        self.reduceBotAsp = nn.Conv2d(inchannels[0], channel, kernel_size=1, bias=False)
        ...
        self.gAwares = [DGCN(channel) for _ in range(self.guideLayers) ]
    ...
    def forward(self, xin, x5,x4,x3,x2):
        ...
        for i in range(self.guideLayers):
            if i == 0:
                ...
                last_seg_feat = F.interpolate(last_seg_feat, size=aspp.size()[2:],mode='bilinear', align_corners=True)  
